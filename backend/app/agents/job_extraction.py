import re

from app.models.schemas import Category, ClassificationResult, EmailInput, JobDashboardExtractionResult

# Constants
_COMPANY_STOPWORDS = {
    "the", "this", "that", "best", "your", "our", "their", "team",
    "dear", "hello", "hi", "hey", "please", "thanks", "regarding",
    "re", "fwd", "fw", "no-reply", "noreply",
}

_FREE_EMAIL_DOMAINS = {
    "gmail", "yahoo", "outlook", "hotmail", "live", "msn", "aol",
    "icloud", "protonmail", "zoho", "yandex", "mail", "inbox",
}


def _clean_value(value: str) -> str:
    cleaned = re.sub(r"\s+", " ", value or "").strip(" -,:;\n\t")
    cleaned = re.sub(
        r"\b(apply link|job alert|opportunity|opening|position|role)\b",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(r"\s{2,}", " ", cleaned).strip(" -,:;\n\t")
    return cleaned


def _first_match(text: str, patterns: list[str], flags: int = re.IGNORECASE) -> str:
    for pattern in patterns:
        match = re.search(pattern, text, flags)
        if match:
            value = (match.group(1) or "").strip()
            if value:
                return value
    return ""


def _is_plausible_company(name: str) -> bool:
    """Reject single common English words that are clearly not company names."""
    stripped = name.strip()
    if not stripped or len(stripped) < 2:
        return False
    return stripped.lower() not in _COMPANY_STOPWORDS


def _extract_company(subject: str, text: str, sender: str) -> str:
    # Tier 1 — explicit job-context patterns in the body (case-insensitive).
    stop = r"(?=\s+(?:as|for|role|position|opening)\b|\s*$)"
    body_patterns = [
        rf"\bfrom\s+([A-Z][A-Za-z0-9&\-]*(?:\s+[A-Z][A-Za-z0-9&\-]*){{0,3}}){stop}",
        rf"\bat\s+([A-Z][A-Za-z0-9&\-]*(?:\s+[A-Z][A-Za-z0-9&\-]*){{0,3}}){stop}",
        rf"\bjoin\s+([A-Z][A-Za-z0-9&\-]*(?:\s+[A-Z][A-Za-z0-9&\-]*){{0,3}}){stop}",
        rf"\bwith\s+([A-Z][A-Za-z0-9&\-]*(?:\s+[A-Z][A-Za-z0-9&\-]*){{0,3}}){stop}",
    ]
    company = _first_match(text, body_patterns, flags=0)
    if company and _is_plausible_company(company):
        return _clean_value(company)

    # Tier 2 — leading portion of the subject before a separator
    subject_match = re.search(
        r"^\s*(?:(?:Re|Fwd?)\s*:\s*)*([A-Za-z0-9&][A-Za-z0-9&\-.]{1,50})\s*[-–:|]",
        subject,
        re.IGNORECASE,
    )
    if subject_match and _is_plausible_company(subject_match.group(1)):
        return _clean_value(subject_match.group(1))

    # Tier 3 — infer from sender email domain
    if "@" in sender:
        domain = sender.split("@", 1)[1].split(".", 1)[0]
        if domain and domain.lower() not in _FREE_EMAIL_DOMAINS:
            return _clean_value(domain.replace("-", " ").title())
    return ""


def _extract_role(subject: str, text: str) -> str:
    role_patterns = [
        r"\b(?:role|position|opening|opportunity)\s*(?:for|:)?\s*([A-Za-z][A-Za-z0-9/&\-\s]{2,80})",
        r"\bfor\s+(?:the\s+)?([A-Za-z][A-Za-z0-9/&\-\s]{2,80})\s+(?:role|position|opening)\b",
        r"\b(?:hiring|interview|application)\s+for\s+([A-Za-z][A-Za-z0-9/&\-\s]{2,80})\b",
    ]
    role = _first_match(text, role_patterns)
    if role:
        return _clean_value(role)

    subject_role = _first_match(
        subject,
        [
            r"[-–:|]\s*([A-Za-z][A-Za-z0-9/&\-\s]{2,80})\s*(?:apply link|job|opening|opportunity)?$",
            r"\bfor\s+([A-Za-z][A-Za-z0-9/&\-\s]{2,80})$",
        ],
        flags=re.IGNORECASE,
    )
    return _clean_value(subject_role)


def _extract_location(text: str) -> str:
    location = _first_match(
        text,
        [
            r"\blocation\s*[:\-]\s*([A-Za-z][A-Za-z,\-/ ]{1,60})",
            r"\bbased in\s+([A-Za-z][A-Za-z,\-/ ]{1,60})",
            r"\b(?:workplace|work location)\s*[:\-]\s*([A-Za-z][A-Za-z,\-/ ]{1,60})",
            r"\b((?:remote|hybrid|on[- ]site))\b",
        ],
    )
    return _clean_value(location)


def _extract_recruiter_name(text: str, sender: str) -> str:
    recruiter = _first_match(
        text,
        [
            r"(?i:\bthis is)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b",
            r"(?i:\bi am)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b",
            r"(?i:(?:regards|best regards|thanks|sincerely)),?\s*\n\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})",
        ],
        flags=0,
    )
    if recruiter:
        return _clean_value(recruiter)

    local_part = sender.split("@", 1)[0]
    name = re.sub(r"[._\-]+", " ", local_part).strip()
    if re.match(r"^[A-Za-z]{2,}(?:\s+[A-Za-z]{2,}){0,2}$", name):
        return name.title()
    return ""


def _extract_interview_datetime(text: str) -> str:
    interview = _first_match(
        text,
        [
            r"\binterview\s+(?:on|at)\s+([^\n\.]{3,100})",
            r"\bscheduled\s+for\s+([^\n\.]{3,100})",
            r"\bmeeting\s+on\s+([^\n\.]{3,100})",
        ],
    )
    return _clean_value(interview)


async def run_job_extraction_agent(email: EmailInput, classification: ClassificationResult) -> JobDashboardExtractionResult:
    """
    Extracts job-related information from the email to update the dashboard.
    """
    if classification.category != Category.job:
        return JobDashboardExtractionResult(is_job_related=False)

    subject = email.subject or ""
    text = f"{subject}\n{email.body or ''}"

    company = _extract_company(subject, text, email.sender or "")
    role = _extract_role(subject, text)
    location = _extract_location(text)
    recruiter_name = _extract_recruiter_name(text, email.sender or "")
    interview_datetime = _extract_interview_datetime(text)

    stage_map = {
        "Application Confirmation": "Applied",
        "Recruiter Outreach": "Screening",
        "Interview Invitation": "Technical Interview",
        "Coding Assessment": "Technical Interview",
        "Offer Letter": "Offer",
        "Rejection": "Rejected",
        "Follow Up": "Screening",
        "Unknown": "Applied",
    }

    stage = stage_map.get(classification.subtype.value, "Applied")
    extracted_fields = sum(
        1
        for value in [company, role, location, recruiter_name, interview_datetime]
        if value
    )
    confidence = min(0.95, 0.55 + extracted_fields * 0.1)

    return JobDashboardExtractionResult(
        is_job_related=True,
        company=company,
        role=role,
        location=location,
        interview_datetime=interview_datetime,
        recruiter_name=recruiter_name,
        stage=stage,
        next_action="Reply and confirm details" if classification.subtype.value in {"Interview Invitation", "Coding Assessment"} else "Track and monitor",
        confidence=confidence,
    )
